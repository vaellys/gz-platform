...
<reps:container layout="true">
	<reps:panel title="" id="top" dock="top" method="post" action="list.mvc" formId="queryForm">
		<reps:footbar>
			<reps:button cssClass="add-a" action="toadd.mvc?id=${item.id }" messageCode="manage.action.add" value="新增"></reps:button>
		</reps:footbar>
	</reps:panel>
	<reps:panel id="mybody" dock="center">
		<reps:grid id="itemList" items="${list}" form="queryForm" var="item" pagination="${pager}" flagSeq="true">
			<reps:gridrow>
				<reps:gridfield title="评分项" width="15" align="center">${item.name}</reps:gridfield>
				...
				<reps:gridfield title="是否可用" width="15" align="center">${item.isEnabled}</reps:gridfield>
